...
def test_pin_read(pull):
    ...
    for p in pin_map:
        pin = Pin(f"GP{str(p)}", mode=Pin.IN, pull=pull)
        # read the pin value
        print(pin())


def test_pin_shorts(pull):
    pull_inverted = Pin.PULL_DOWN if pull == Pin.PULL_UP else Pin.PULL_UP
    # enable all pulls of the specified type
    for p in pin_map:
        pin = Pin(f"GP{str(p)}", mode=Pin.IN, pull=pull_inverted)
    # then change the pull one pin at a time and read its value
    i = 0
    while i < len(pin_map):
        pin = Pin(f"GP{str(pin_map[i])}", mode=Pin.IN, pull=pull)
        Pin(f"GP{str(pin_map[i - 1])}", mode=Pin.IN, pull=pull_inverted)
        i += 1
        # read the pin value
        print(pin())
